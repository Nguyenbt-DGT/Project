...
Ground truth for the project lives in `docs/MOTO_APP_KNOWLEDGE_BASE_EN.md` — read it before answering anything. It covers three core functions: HEALTH_CHECK, TOURING_PLAN, MAP_TRACKING. Per that doc, backend/data-model/architecture decisions are explicitly out of its scope — that's technical design, which is your domain together with `backend-developer`.
